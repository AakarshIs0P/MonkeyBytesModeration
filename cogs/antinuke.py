"""
cogs/antinuke.py  --  Paladin Protection System
===============================================
...
class AntiNuke(commands.Cog):
    """Paladin's antinuke service and shared protection state."""
    ...
    def _is_protected(self, guild_id: int, user_id: int, *, scope: str = None) -> bool:
        ...
        if scope is not None:
            ...
            now = time.time()
            expires_at = bypass.get("valid_until") if bypass else None
            idle_expires_at = bypass.get("idle_expires_at") if bypass else None
            if bypass and (
                (expires_at is None or now < expires_at)
                and (idle_expires_at is None or now < idle_expires_at)
                and (bypass["scope"] == "both" or bypass["scope"] == scope)
            ):
                return True
        return False
    ...
    def get_alert_subscribers(self, guild_id: int) -> tuple[int, ...]:
        """Return a read-only snapshot of this server's AlertNuke subscribers."""
        return tuple(self._alertnuke_list(guild_id))
    ...
    def _track_pending_object(self, guild_id: int, action: str, user_id: int, obj) -> None:
        """Keep every reversible object in the current rolling-window incident."""
        if obj is None:
            return
        now = time.monotonic()
        entries = self._pending_obj.setdefault(guild_id, {}).setdefault(action, {}).setdefault(user_id, [])
        entries.append((now, obj))
        window = self._an_window(guild_id)
        entries[:] = [(created_at, item) for created_at, item in entries if now - created_at <= window]

    def _take_pending_objects(self, guild_id: int, action: str, user_id: int) -> list:
        """Return and clear only this actor's reversible objects."""
        action_entries = self._pending_obj.get(guild_id, {}).get(action, {})
        entries = action_entries.pop(user_id, [])
        if not action_entries and guild_id in self._pending_obj:
            self._pending_obj[guild_id].pop(action, None)
        if guild_id in self._pending_obj and not self._pending_obj[guild_id]:
            del self._pending_obj[guild_id]
        return [item for _, item in entries]

    async def _revert_pending_objects(
        self, guild: discord.Guild, actor: discord.Member, action: str
    ) -> bool:
        """Reverse all tracked objects for one actor's current incident."""
        pending_objs = self._take_pending_objects(guild.id, action, actor.id)
        reversion_cog = self.bot.cogs.get("Reversion")
        if not reversion_cog:
            return False
        try:
    ...
    async def _an_punish(self, guild: discord.Guild, actor: discord.Member, action: str, count: int):
        ...
        # ------------------------------------------------------------------
        if action == "bot_add":
            pending_bots = self._take_pending_objects(guild.id, "bot_add", actor.id)
    ...
    async def _register_attempt(self, guild: discord.Guild, actor: discord.Member, action: str, obj=None):
        ...
        self._track_pending_object(guild.id, action, actor.id, obj)
    ...
    async def pre_check_mod_action(
        ...
        if current_count + 1 >= threshold:
            ...
            reversed_ok = await self._revert_pending_objects(guild, actor, action)
            alert_embed = self._alertnuke_antinuke_embed(guild, actor, action, projected, roles_to_remove, reversed_ok)
            await self._alertnuke_dm(guild, alert_embed)
    ...
    async def check_mod_action(
        ...
        if current_count >= threshold:
            ...
            reversed_ok = await self._revert_pending_objects(guild, actor, action)
            alert_embed = self._alertnuke_antinuke_embed(guild, actor, action, current_count, roles_to_remove, reversed_ok)
            await self._alertnuke_dm(guild, alert_embed)
        ...
        for attempt in range(4):
            try:
                ...
                async for entry in guild.audit_logs(limit=5, action=audit_action):
                    ...
                    # Make sure the audit entry belongs to the dispatched object.
                    # Webhook events dispatch a channel, whereas the audit target is
                    # the webhook itself, so match its channel instead of its ID.
                    if obj is not None and getattr(entry, "target", None):
                        if action_name == "webhook":
                            ...
                            target_channel_id = target_channel_id or getattr(extra_channel, "id", None)
                            if target_channel_id != getattr(obj, "id", None):
                                continue
                        elif getattr(obj, "id", None) != entry.target.id:
                            continue
        ...
        for attempt in range(4):
            ...
            try:
                async for entry in guild.audit_logs(limit=5, action=discord.AuditLogAction.kick):
                    ...
                    if (discord.utils.utcnow() - entry.created_at).total_seconds() > 15: continue
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        # Trusted inviters bypass antinuke, including bot additions.
    ...
    async def on_guild_emojis_update(self, guild: discord.Guild, before, after):
        if len(before) > len(after):
            ...
            await self._audit_strike(
                guild, discord.AuditLogAction.emoji_delete, "emoji_delete", occurrences=deleted_count
            )
        elif len(after) > len(before):
            created_count = len(after) - len(before)
            await self._audit_strike(
                guild, discord.AuditLogAction.emoji_create, "emoji_create", occurrences=created_count
            )
    ...
    async def on_guild_stickers_update(self, guild: discord.Guild, before, after):
        if len(before) > len(after):
            deleted_count = len(before) - len(after)
            await self._audit_strike(
                guild, discord.AuditLogAction.sticker_delete, "sticker_delete", occurrences=deleted_count
            )
        elif len(after) > len(before):
            created_count = len(after) - len(before)
            await self._audit_strike(
                guild, discord.AuditLogAction.sticker_create, "sticker_create", occurrences=created_count
            )
    ...
    async def _am_punish(self, guild: discord.Guild, member: discord.Member, strike: int, cfg: dict, channel: discord.TextChannel = None, trigger: str = "spam"):
        ...
        if strike == 1:
            ...
            if automod_cog:
                automod_cog.automod_acted.setdefault(guild.id, set()).add(member.id)
            try:
                await member.kick(reason=f"[Paladin AutoMod] {trigger.title()} -- Strike 3")
            except discord.Forbidden:
                if automod_cog:
                    automod_cog.automod_acted[guild.id].discard(member.id)
    ...
    async def _key_expiry_task(self):
        ...
        expired = [
            ...
            if (
                (v["valid_until"] is not None and now >= v["valid_until"])
                or (v.get("idle_expires_at") is not None and now >= v["idle_expires_at"])
            )
        ]
        for k in expired:
    ...
    async def usekey(self, ctx: commands.Context, *, key: str):
        ...
        self._key_bypass[(ctx.guild.id, ctx.author.id)] = {
            ...
            "idle_expires_at": key_data["idle_expires_at"],
            "key":         key_data["key"],
        }
