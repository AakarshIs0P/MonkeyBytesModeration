...
class Logging(commands.Cog):
    ...
    async def on_member_remove(self, member: discord.Member):
        ...
        automod = self.bot.cogs.get("AutoMod")
        if automod and member.id in automod.automod_acted.get(member.guild.id, set()):
            automod.automod_acted[member.guild.id].discard(member.id)
            return
    ...
    async def on_member_ban(self, guild: discord.Guild, user: discord.User):
        ...
        automod = self.bot.cogs.get("AutoMod")
        if automod and user.id in automod.automod_acted.get(guild.id, set()):
            automod.automod_acted[guild.id].discard(user.id)
            return
    ...
    # ── Paladin punishment log (called from antinuke.py) ──────────────
